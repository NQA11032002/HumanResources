...
<a href="#" class="nav-item d-flex nav-item__post position-relative">
    <div href="" class="nav-item__image home-item--image">
        <img src="{{ asset($item->logo) }}" alt="">
    </div>

    <div class="nav-item__content d-flex justify-content-between position-relative">
        <div class="nav-item__content--left">
            <p href="#" class="h4">{{ $item->title }}</p>
            <p href="#" class="h6">{{ $item->name }}</p>
            <p class="salary"><i class="fa-solid fa-dollar-sign"></i> Lương:
                <span class="salary-content">{{ number_format($item->salary_from, 0, ',', '.') }}đ -
                    {{ number_format($item->salary_to, 0, ',', '.') }}đ</span>
            </p>
